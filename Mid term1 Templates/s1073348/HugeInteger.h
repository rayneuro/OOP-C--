...
HugeInteger< T > HugeInteger< T >::square()
{
    ...
    {
        k = i;
        for (int j = 0; j < integer.size() ; j++,k++)
        {
            square.integer[k] += integer[i] * integer[j];
        }
    }
    for(int i=0;i < square.integer.size()-1;i++)
    {
        int ten = square.integer[i] / 10;
        square.integer[i] %= 10;
        square.integer[i + 1] += ten;
    }
    typename T::iterator it = square.integer.end() - 1;
