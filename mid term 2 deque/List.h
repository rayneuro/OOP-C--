...
public:
		...
		: myData()
	{
		...
		node * t1 = myData.myHead;
		node * t2 = right.myData.myHead->next;
		...
		t1->next = myData.myHead; myData.myHead->prev = t1;
	}
	...
	void insert(const_iterator where, const value_type& val) // insert val at where
	{
		...
		temp->myVal = val;
		temp->prev = t1.ptr->prev;
		temp->next = t1.ptr;
		t1.ptr->prev->next = temp;
		t1.ptr->prev = temp;
	...
	void erase(const const_iterator where)
	{
		if (myData.mySize > 0) {
			...
			t1.ptr->prev->next = t1.ptr->next;
			t1.ptr->next->prev = t1.ptr->prev;
			delete t1.ptr;
		}
		myData.mySize--;
	}
